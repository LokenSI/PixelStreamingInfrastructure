...
function sendRetryResponse(res) {
  ...
  let html = fs.readFileSync(`${htmlDirectory}/queue/queue.html`, {
    encoding: "utf8",
  })
...
function getAvailableCirrusServer() {
  for (cirrusServer of cirrusServers.values()) {
    console.log(
      `Checking for ${cirrusServer.address}:${cirrusServer.port} with clientConnected ${cirrusServer.numConnectedClients}`
    )
    if (cirrusServer.numConnectedClients === 0 && cirrusServer.ready === true) {
      ...
      if (cirrusServer.lastRedirect) {
        if ((Date.now() - cirrusServer.lastRedirect) / 1000 < 45) {
          console.log(
            `lastRedirect was less than 45 seconds so ignoring this server with the difference ${
              (Date.now() - cirrusServer.lastRedirect) / 1000
            }`
          )
          continue
        }
...
function setAuthenticationCookie(req, res) {
  try {
    ...
    if (config.enableAuthentication) {
      ...
      res.cookie("mm_authenticated", "true", {
        maxAge: config.authCookieExpirationMaxAge,
        httpOnly: true,
        domain: fulldomain,
      })
    }
    ...
    console.log(
      `ERROR (${e.toString()}): Issue with parsing domain and cookie creation`
    )
    ai.logError(e)
  }
}
...
if (enableRedirectionLinks) {
  ...
  app.get("/", isAuthenticated("/login"), function (req, res) {
    ...
    if (cirrusServer != undefined) {
      ...
      console.log(
        `Redirect to ${cirrusServer.enableHttps ? "https" : "http"}://${
          cirrusServer.address
        }:${cirrusServer.port}`
      )
      res.redirect(
        `${cirrusServer.enableHttps ? "https" : "http"}://${
          cirrusServer.address
        }:${cirrusServer.port}/`
      )
    } else {
  ...
  app.get("/custom_html/:htmlFilename", (req, res) => {
    ...
    if (cirrusServer != undefined) {
      ...
      res.redirect(
        `${cirrusServer.enableHttps ? "https" : "http"}://${
          cirrusServer.address
        }:${cirrusServer.port}/custom_html/${req.params.htmlFilename}`
      )
    } else {
      sendRetryResponse(res)
    }
    ...
    if (message.type === "connect") {
      ...
      if (!server || server.size <= 0) {
        console.log(
          ...
          } with playerConnected: ${message.playerConnected} and version: ${
            message.version
          }`
        )
      ...
      if (cirrusServer) {
        ...
        ai.logEvent(
          "CirrusServerUndefined",
          `No cirrus server found on streamer connect: ${connection.remoteAddress}`
        )
        disconnect(connection)
      }
      ...
      // The stream connects to a Cirrus server and so is ready to be used
      console.log(
        `Received request to streamerDisconnected from remote address ${connection.remoteAddress}`
      )
      cirrusServer = cirrusServers.get(connection)
      if (cirrusServer) {
        ...
        ai.logEvent(
          "CirrusServerUndefined",
          `No cirrus server found on streamer disconnect: ${connection.remoteAddress}`
        )
        disconnect(connection)
      }
      ...
      console.log(
        `Received request to connect to remote address ${connection.remoteAddress}`
      )
      cirrusServer = cirrusServers.get(connection)
      if (cirrusServer) {
        console.log(
          `Clients already connected to ${cirrusServer.address}:${cirrusServer.port} are ${cirrusServer.numConnectedClients}`
        )
        cirrusServer.numConnectedClients++
        console.log(
          `Clients connection for ${cirrusServer.address}:${cirrusServer.port} increased to ${cirrusServer.numConnectedClients}`
        )
        ...
        ai.logEvent(
          "CirrusServerUndefined",
          `No cirrus server found on client connect: ${connection.remoteAddress}`
        )
      ...
      console.log(
        `Received request to disconnect to remote address ${connection.remoteAddress}`
      )
      cirrusServer = cirrusServers.get(connection)
      if (cirrusServer) {
        console.log(
        ...
        console.log(
          `Client connected to Cirrus server ${cirrusServer.address}:${cirrusServer.port} decreased to ${cirrusServer.numConnectedClients}`
        )
        ...
        ai.logEvent(
          "CirrusServerUndefined",
          `No cirrus server found on client disconnect: ${connection.remoteAddress}`
        )
